...
<li class="breadcrumb-item">
    <a href="{{ route('admin.post.index') }}" title="{{ trans('icore::posts.route.index') }}">
        {{ trans('icore::posts.route.index') }}
    </a>
</li>
<li class="breadcrumb-item active" aria-current="page">{{ trans('icore::posts.route.edit') }}</li>
@endsection
            ...
            <div class="col-lg-3">
                <div class="form-group">
                    <div class="custom-control custom-checkbox">
                        ...
                        <input type="checkbox" class="custom-control-input" id="seo_noindex" 
                        name="seo_noindex" value="{{ $post::SEO_NOINDEX }}" {{ (old('seo_noindex', $post->seo_noindex) == $post::SEO_NOINDEX) ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    <div class="custom-control custom-checkbox">
                        ...
                        <input type="checkbox" class="custom-control-input" id="seo_nofollow" 
                        name="seo_nofollow" value="{{ $post::SEO_NOFOLLOW }}" {{ (old('seo_nofollow', $post->seo_nofollow) == $post::SEO_NOFOLLOW) ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select class="custom-select" data-toggle="collapse" aria-expanded="false" aria-controls="collapsePublishedAt" id="status" name="status">
                        <option value="{{ $post::ACTIVE }}" {{ (old('status', $post->status) == $post::ACTIVE) ? 'selected' : '' }}>
                            {{ trans('icore::filter.active') }}
                        </option>
                        <option value="{{ $post::INACTIVE }}" {{ (old('status', $post->status) == $post::INACTIVE) ? 'selected' : '' }}>
                            {{ trans('icore::filter.inactive') }}
                        </option>
                        <option value="{{ $post::SCHEDULED }}" {{ (old('status', $post->status) == $post::SCHEDULED) ? 'selected' : '' }}>
                            {{ trans('icore::filter.scheduled') }}
                        </option>
                    </select>
                </div>
                <div class="form-group collapse {{ (old('status', $post->status) != $post::INACTIVE) ? 'show' : '' }}" id="collapsePublishedAt">
                    <label for="published_at">
                        {{ trans('icore::posts.published_at.label') }} <i data-toggle="tooltip" data-placement="top" title="{{ trans('icore::posts.published_at.tooltip') }}" class="far fa-question-circle"></i>
                    </label>
                    <div id="published_at">
                        <div class="form-group">
