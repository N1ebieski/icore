...
namespace N1ebieski\ICore\Http\Clients\AI\OpenAI\Responses;
...
class ChatCompletionResponse extends Response implements ChatCompletionResponseInterface
{
    public function getData(): string
    {
        if (empty($this->parameters->choices)) {
            throw new \N1ebieski\ICore\Exceptions\AI\EmptyChoiceException();
        }

        if (empty($this->parameters->choices[0]->message->content)) {
            throw new \N1ebieski\ICore\Exceptions\AI\EmptyMessageException();
        }
    ...
    public function getDataAsArray(): array
    {
        ...
        if ($json === null) {
            throw new \N1ebieski\ICore\Exceptions\AI\InvalidJsonException();
        }
